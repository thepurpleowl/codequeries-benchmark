...
class UseImplicitNoneReturnValue(BaseContexts):
    """
    ...
    def create_used_func_calls(self, message, start_line, start_column, end_line, end_column):
        """
        Args:
    ...
    def get_used_functions(self, file_path, aux_result_df):
        ...
        """
        used_import_df = aux_result_df.loc[(aux_result_df["Name"] == 'Used function call map') & (aux_result_df["Path"] == file_path),
                                           __columns__]
        ...
        used_import_df["call_map"] = (used_import_df.apply(
                                      lambda x: self.create_used_func_calls(x.Message,
...
def get_query_specific_context(program_content, parser, file_path, message, result_span, aux_result_df):
    ...
    context_object = UseImplicitNoneReturnValue(parser)
    ...
    for block in all_blocks:
        ...
        if(block.block_type == CLASS_FUNCTION):
            ...
            for call_map in used_func_calls:
                used_func_name = call_map.func_name.split('.')[-1]
                used_func_class = call_map.func_name.split('.')[0]  # enclosing class in case of inner class
            ...
            for call_map in used_func_calls:
                used_func_name = call_map.func_name
                if(block_func_name == used_func_name
                        and (call_map.start_line in local_block_lines
                             or call_map.end_line in local_block_lines)):
                    add_block = True
